...
class TwitchChat {
  ...
  connect() {
    ...
    this.ws.on('open', () => {
        ...
        // Retrieve the OAuth token and channel name from storage
        chrome.storage.sync.get(['oauthToken', 'channelName'], function(data) {
            if (chrome.runtime.lastError) {
                console.error('Error retrieving OAuth token and channel name:', chrome.runtime.lastError);
                return;
            }
